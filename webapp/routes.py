...
def get_station_availability_averages(station_property, station):
    ...
    for day in weekdays:
        rows = g._database.execute(
            f"select ROUND(avg({station_property}), 0), hour(lastUpdate) from station_updates where stationId = {station} and weekday(lastUpdate) = {day} group by hour(lastUpdate)  order by hour(lastUpdate) asc;"
        )
        availability_history[day] = []
        for row in rows:
            availability_history[day].append(
                {f"{station_property}": int(row[0]), "hour": row[1]}
            )
...
def weatherWidget_js():
    return render_template(
        "weatherWidget.js", openweather_api=app.config["OPENWEATHER_API_KEY"]
    )
...
def get_weather_forecast_and_prediction_for_station():
    ...
    if (
        not station_latitude
        or not station_longitude
        or not time_requested_unix_timestamp
        or not station_number
    ):
        abort(400)
    ...
    weather_forecast_api_response = requests.get(
        f"https://api.openweathermap.org/data/2.5/onecall?lat={station_latitude}&lon={station_longitude}&exclude=current,minutely,daily&units=metric&appid={OPENWEATHER_API_KEY}"
    )
    if weather_forecast_api_response.status_code == 200:
        logging.info(
            "Request to Openweather Forecast API succeeded with status code 200"
        )
        hourly_weather_forecast = weather_forecast_api_response.json()
        if not "hourly" in hourly_weather_forecast:
            logging.error(
                f"OPENWEATHER Forecast API did not return expected data: {hourly_weather_forecast}."
            )
            abort(400)
        else:
            for i in range(len(hourly_weather_forecast["hourly"])):
                if (
                    hourly_weather_forecast["hourly"][i]["dt"]
                    == time_requested_unix_timestamp
                ):
                    # get the weather variables from the api that correspond to the ones we are also storing in our (historical data) database
                    main_weather = hourly_weather_forecast["hourly"][i]["weather"][0][
                        "main"
                    ]
                    ...
                    weather_icon = hourly_weather_forecast["hourly"][i]["weather"][0][
                        "icon"
                    ]
                    ...
                    station_model_unpickled = pickle.load(
                        open(
                            f"./station_models/randomForest_stationID_{station_number}",
                            "rb",
                        )
                    )
                    ...
                    if day_of_the_week_string != "Friday":
                        features_and_values_dict[
                            f"weekday_{day_of_the_week_string}"
                        ] = 1
                    ...
                    new_sample_to_make_prediction = v.fit_transform(
                        features_and_values_dict
                    )

                    # predict availability for user input and weather forecast
                    availability_prediction_result = station_model.predict(
                        new_sample_to_make_prediction
                    )
            ...
            return {
                "error": f"time requested didn't match available data: {hourly_weather_forecast['hourly']}"
            }
    else:
        logging.error(
            f"Request to Openweather Forecast API failed with {weather_forecast_api_response.status_code}: {weather_forecast_api_response.reason}"
        )
        abort(400)
