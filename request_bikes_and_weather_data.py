...
def call_bikes_api():
    bikes_api_response = requests.get(
        f"https://api.jcdecaux.com/vls/v1/stations?contract=dublin&apiKey={BIKES_API_KEY}"
    )
    if bikes_api_response.status_code == 200:
        ...
        logging.error(
            f"Request to JCDecaux API failed with {bikes_api_response.status_code}: {bikes_api_response.reason}"
        )
        return None
...
def call_weather_api(latitude, longitude):
    ...
    if weather_api_response.status_code == 200:
        ...
        logging.error(
            f"Request to OPENWEATHER API failed with {weather_api_response.status_code}: {weather_api_response.reason}"
        )
        return None
...
def create_station_row_in_db(station):
    ...
    try:
        ...
        session.rollback()
        logging.error(
            f"Failed to add station {new_station.__dict__} to the STATIONS table with error: {e}"
        )
...
def create_station_update_row_in_db(station, weather_data, last_update_datetime):
    ...
    new_station_update = StationUpdates(
        stationId=station["number"],
    ...
    try:
        ...
        session.rollback()
        logging.error(
            f"Failed to add update for station {new_station_update.__dict__} to the STATION_UPDATES table with error: {e}"
        )
...
while True:
    try:
        ...
        if json_folder_exist:
            with open(
                f"{json_folder}/dublin-bikes-{datetime.now()}.json", "w"
            ) as outfile:
                logging.info(
                    f"Writing bikes output to dublin-bikes-{datetime.now()}.json"
                )
        ...
        for station in stations_json:
            ...
            try:
                ...
                last_update_timestamp_seconds = math.floor(
                ...
                update_row_exist = session.query(StationUpdates).get(
                    (station["number"], last_update_datetime)
                )
                if not update_row_exist:
                    ...
                    if json_folder_exist:
                        with open(
                            f"{json_folder}/weather-stationID-{station['number']}-{last_update_datetime}.json",
                            "w",
                        ) as outfile:
                            logging.info(
                                f"Writing weather output to weather-stationID-{station['number']}-{last_update_datetime}.json"
                            )
                            json.dump(weather_data, outfile)

                    # insert station availability and weather updates in station_updates table
                    create_station_update_row_in_db(
                        station, weather_data, last_update_datetime
                    )
            except Exception as e:
                logging.error(e)
